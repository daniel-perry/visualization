...
def showRenderTime(caller, something):
  renderTime = caller.GetLastRenderTimeInSeconds()
  fps = 1.0 / renderTime
  print renderTime,',',fps

# sample distance, renderTime (s) , fps
# 0.01, 23, 0.03 # zoomed
# 0.01, 12, 0.07
# 0.1, 3.2, 0.3 # zoomed
# 0.1, 1.4, 0.73
...
def main(argv):
  if len(argv) < 2:
    print "usage:",argv[0]," data.vtk sampleDistance"
    exit(1)
  data_fn = argv[1]
  sampleDistance = float(argv[2])
  ...
  renderer.AddObserver( vtk.vtkCommand.EndEvent, showRenderTime )
